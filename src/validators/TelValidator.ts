import { BaseValidator } from './BaseValidator';
import { ValidationResult, ValidatorOptions } from '../types';

/**
 * 電話番号バリデーター
 */
export class TelValidator extends BaseValidator {
    private patterns = {
        telWithHyphens: /^0(((\d{1}-\d{4}|\d{2}-\d{3,4}|\d{3}-\d{2,3}|\d{4}-\d{1})-\d{4})|\d{3}-\d{3}-\d{3})$/,
        telWithoutHyphens: /^0\d{9,10}$/,
        halfWidth: /^[0-9\-]+$/
    };

    private defaultMessages = {
        default: '電話番号の形式が正しくありません。',
        withHyphens: '電話番号はハイフン付きの形式で入力してください。',
        withoutHyphens: '電話番号はハイフンなしの形式で入力してください。',
        halfWidth: '半角数字で入力してください。'
    };

    /**
     * 電話番号の形式を検証
     */
    async validate(value: any, options?: ValidatorOptions): Promise<ValidationResult> {
        // 空の場合はスキップ（RequiredValidatorで処理）
        if (this.isEmpty(value)) {
            return await this.handleNext(value, options);
        }

        // 文字列または数値に変換
        const conversionResult = this.convertToString(value, 'tel', this.defaultMessages.default, options);
        if (!conversionResult.success) {
            return conversionResult.result;
        }
        const stringValue = conversionResult.value;

        // 全角数字を半角に変換
        const normalizedValue = this.normalizeNumber(stringValue);

        // 半角数字チェック
        if (!this.patterns.halfWidth.test(normalizedValue)) {
            const message = this.createErrorMessage('tel-halfwidth', this.defaultMessages.halfWidth, options);
            return this.createFailureResult([
                this.createError('tel-halfwidth', message, value)
            ]);
        }

        // ハイフンの許可設定を取得
        const allowHyphens = options?.allowHyphens ?? null;

        // 電話番号形式のチェック
        let isValid = false;
        let errorMessage = this.defaultMessages.default;

        if (allowHyphens === true) {
            isValid = this.patterns.telWithHyphens.test(normalizedValue);
            errorMessage = this.defaultMessages.withHyphens;
        } else if (allowHyphens === false) {
            isValid = this.patterns.telWithoutHyphens.test(normalizedValue);
            errorMessage = this.defaultMessages.withoutHyphens;
        } else {
            // null の場合は両方許可
            isValid = this.patterns.telWithHyphens.test(normalizedValue) || 
                     this.patterns.telWithoutHyphens.test(normalizedValue);
        }

        if (!isValid) {
            const message = this.createErrorMessage('tel', errorMessage, options);
            return this.createFailureResult([
                this.createError('tel', message, value)
            ]);
        }

        // 検証成功、次のバリデーターへ
        return await this.handleNext(normalizedValue, options);
    }

    /**
     * 全角数字を半角に変換
     */
    private normalizeNumber(value: string): string {
        return value
            .replace(/[０-９]/g, (s) => String.fromCharCode(s.charCodeAt(0) - 0xfee0))
            .replace(/[ー－]/g, '-');
    }
}
